...
rm -f output* $6 *.raw *.wav merged.mp4 almost_done.mp4 output_dwatermarked.mp4 seriously_imathere.mp4 final_audio.mp3 final_thumbnail.png thumbnail_right.png
echo -e "\n\e[32mGenerating Images with guidolib\e[39m\n" && videogeneration $1 $2 && \
    ...
    (([ "$5" != "" ] && echo -e "\n\e[32mMerging solo recording mp3 and accompaniment mp3\e[39m\n" && ffmpeg -i $5 -i out_pitched.wav -shortest -filter_complex amix=inputs=2:duration=shortest final_audio.mp3) || ([ "$5" == "" ] && echo -e "\n\e[32mMerging solo synth wav and accompaniment mp3\e[39m\n" && ffmpeg -i out_pitched.wav -i audio.wav -shortest final_audio.mp3)) && \
    echo -e "\n\e[32mAdding audio to generated video\e[39m\n" && ffmpeg -i output_watermarked.mp4 -i final_audio.mp3 -shortest -c copy almost_done.mp4 && \
    ...
    echo -e "\n\e[32mAdding text watermark\e[39m\n" && ffmpeg -i merged.mp4 -vf drawtext="fontfile=HelveticaNeueBd.ttf: text='Metronaut App': fontcolor=0x00000060: fontsize=20: x=(w-200): y=(h-text_h - 10)" -codec:a copy seriously_imathere.mp4 && \
    echo -e "\n\e[32mAdding qrcode watermark on video\e[39m\n" && ffmpeg -i seriously_imathere.mp4 -i qrcode.png -filter_complex "overlay=W-200:H-200" $6
