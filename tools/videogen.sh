...
rm -f output* $5 *.raw *.wav merged.mp4 almost_done.mp4 final_audio.mp3
...
echo -e "\n\e[32mGenerating Images with guidolib\e[39m\n" && videogeneration $1 $2 && \
    ...
    (([ "$4" == "0" ] && cp out.wav out_pitched.wav) ||
         ([ "$4" -ne "0" ] && echo -e "\n\e[32mPitch shift accomp\e[39m\n" && sox out.wav out_pitched.wav pitch $4 20 19)) && \
    echo -e "\n\e[32mMerging solo wav and accompaniment mp3\e[39m\n" && ffmpeg -i audio.wav -i out_pitched.wav -filter_complex amix=inputs=2:duration=longest final_audio.mp3 && \
    ...
    echo -e "\n\e[32mAdding text watermark\e[39m\n" && ffmpeg -i merged.mp4 -vf drawtext="fontfile=HelveticaNeueBd.ttf: text='Metronaut App': fontcolor=0x00000060: fontsize=20: x=(w-text_w - 10): y=(h-text_h - 10)" -codec:a copy $5
